<?php
namespace Themes\Moderna\Presenter;
...
class FooterMenuLinksPresenter extends Presenter
{
    ...
    {
        return PHP_EOL.'<ul class="icon-list link">'.PHP_EOL;
    }
    ...
    {
        return '<li'.$this->getActiveState($item).'><a href="'.$item->getUrl().'" '.$item->getAttributes().'>'.$item->getIcon().' <i class="fa fa-angle-right"></i>'.$item->title.'</a></li>'.PHP_EOL;
    }
